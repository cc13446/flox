...
class TemplateTest extends Specification {
    ...
    def "test"() {
        ...
        """select * from user <where><choose><when test='id!= null'> and id = #{id} </when><when test='name!= null'> and name = #{name} </when></choose></where>"""       | ["id": "11", "name": "cc"]     | """select * from user WHERE id = ? """                    | [11]
        """select * from user <where> id in <foreach item="item" index="index" collection="list" open="(" separator="," close=")"> #{item}_#{index} </foreach></where>""" | ["list": ["11", "name", "cc"]] | """select * from user WHERE id in ( ?_? , ?_? , ?_? ) """ | [11, 0, "name", 1, "cc", 2]
    }
}
